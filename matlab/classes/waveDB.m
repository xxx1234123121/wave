classdef waveDB
    ...
    properties
        ...
        vectorSelectTemplate = ['SELECT  ',...
         ...
         downsampleSelectTemplate = ['WITH ordered3 AS (',...
            'WITH ordered2 AS (',...
            'WITH ordered1 AS (',...
            'SELECT ',...
            'row_number() over ',...
            '(order by ST_Y(###PREFIX###location),ST_X(###PREFIX###location)) as rownum1,',...
            '  ###PREFIX###datetime,         ',...
            '  ###PREFIX###speed,            ',...
            '  ###PREFIX###direction,        ',...
            '  ST_X(###PREFIX###location),   ',...
    ...
    methods
        ...
        function dField = selectWind(db,source,tBegin,tEnd,location)
            ...
            sql = [regexprep(regexprep(db.vectorSelectTemplate,'###PREFIX###','tblwind.win'),...
            '###TABLE###','public.tblwind'),...
        ...
        function dField = selectCurrent(db,source,tBegin,tEnd,location)
            ...
            sql = [regexprep(...
            regexprep(db.downsampleSelectTemplate,'###PREFIX###','cur.cur'),...
            '###TABLE###','public.tblcurrent cur'),...
                ' AND src.srcname = ''',source,''''];
            
            if(nargin>2)
                sql = [sql,' AND cur.curdatetime >= ''',...
                    datestr(tBegin,'yyyy-mm-dd HH:MM:SS'),''''];
            end
            if(nargin>3)
                sql = [sql,' AND cur.curdatetime <= ''',...
            ...
            sql = [sql,') SELECT * FROM ordered1 WHERE rownum1%100=0',...
                ') SELECT row_number() over ',...
                '(order by ordered2.st_x, ordered2.st_y) as rownum2, ',...
                'ordered2.* FROM ordered2) SELECT * FROM ordered3 ',...
                'WHERE rownum1%100=0'];
        ...
        function dField = selectSpectra(db,source,tBegin,tEnd,location)
            ...
            sql = [sql,' ORDER BY ST_Y(tblwave.wavlocation),ST_X(tblwave.wavlocation) OFFSET 10000'];
